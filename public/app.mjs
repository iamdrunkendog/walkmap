...
function renderMode(){
  $('pan').classList.toggle('active',mode==='pan');$('draw').classList.toggle('active',mode==='draw');$('draw').hidden=Boolean(drawing);$('finish').hidden=!drawing;$('cancel').hidden=!(drawing||mode.startsWith('visit'));
  ...
  for(const id of ['course-name','region','tags','speed','duplicate','visit-add'])$(id).disabled=Boolean(drawing);
...
function renderLibrary(){
  ...
  $('course-list').innerHTML=items.map(c=>`<button class="library-card" data-id="${c.id}"><strong>${esc(c.name)}</strong><small>${esc(c.region||'지역 미입력')} ${c.tags.map(t=>'#'+esc(t)).join(' ')}<br>${(timings(c).meters/1000).toFixed(2)} km · 방문 ${c.visits.length}곳 · ${Math.ceil(timings(c).total)}분</small></button>`).join('');
...
$('search-form').onsubmit=async e=>{
  ...
  try{
    ...
    $('search-state').textContent=items.length?'네이버 검색 결과 · 선택하면 지도만 이동합니다.':'검색 결과가 없습니다. 지역명과 장소명을 함께 입력해 보세요.';
