...
export function emptyCourse() {
  return {id: crypto.randomUUID(), version: 0, name: '새로운 산책 코스', region: '', tags: [], speed: 4, points: [], visits: []};
}
...
export function validateCourse(c) {
  ...
  return {id:c.id,version:c.version,name:str(c.name,100,true),region:str(c.region,100),tags:c.tags.map(v=>str(v,40,true)),speed:number(c.speed,0.5,10),points:c.points.map(pos),visits};
}
